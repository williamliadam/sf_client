...
export const dayPlanSlice = createSlice({
	...
	reducers: {
		...
		addDishToMenu: (
			state,
			action: PayloadAction<{ dish: DishType; menuId: string }>,
		) => {
		...
		deleteMenuFromDayPlan: (state, action: PayloadAction<string>) => {
			state.dayPlan.menus = state.dayPlan.menus.filter(
		...
		insertDishToMenu: (
			state,
			action: PayloadAction<{ dish: DishType; menuId: string; index: number }>,
		) => {
		...
		deleteDishFromMenu: (
			state,
			action: PayloadAction<{ dishId: string; menuId: string }>,
		) => {
		...
		addMenuToDayPlan: (state, action: PayloadAction<MenuType>) => {
			const newMenu = { ...action.payload, id: createId() };
